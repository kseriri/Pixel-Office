...
import { CodexScanner } from './codexScanner.js';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // [検証用パッチ] グローバル ~/.claude を汚さないよう hooks を無効化し、
    // 観察ベース(JSONL)で全セッションを監視する。
    ...
    // Also observe Codex CLI sessions (~/.codex/sessions) alongside Claude.
    const codexScanner = new CodexScanner(store, store.nextAgentId);
    codexScanner.start();

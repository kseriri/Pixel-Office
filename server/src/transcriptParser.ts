...
export function processTranscriptLine(
  ...
  try {
    ...
    // -- Model + token usage extraction from assistant records only --
    const recordModel =
      record.type === 'assistant' ? (record.message?.model as string | undefined) : undefined;
    if (typeof recordModel === 'string' && recordModel.length > 0) {
      agent.model = recordModel;
    }
    const usage = (record.type === 'assistant' ? record.message?.usage : undefined) as
      | {
          input_tokens?: number;
    ...
    if (usage) {
      ...
      if (typeof usage.cache_read_input_tokens === 'number') {
        agent.cacheReadTokens = (agent.cacheReadTokens ?? 0) + usage.cache_read_input_tokens;
      }
      if (typeof usage.cache_creation_input_tokens === 'number') {
        agent.cacheWriteTokens = (agent.cacheWriteTokens ?? 0) + usage.cache_creation_input_tokens;
      }
      agents.broadcast({

...
export interface AgentState {
  ...
  inputTokens: number;
  outputTokens: number;
  /** Cumulative cache-read input tokens (billed at ~0.1x input rate) */
  cacheReadTokens?: number;
  /** Cumulative cache-creation (write) input tokens (billed at ~1.25x input rate) */
  cacheWriteTokens?: number;

  // -- AI model (from JSONL assistant records, e.g. "claude-fable-5") --
  model?: string;

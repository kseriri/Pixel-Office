...
/**
 * Load pre-colored character sprites from assets/characters/. Scans for every
 * char_N.png (any N, sorted numerically) so dropping in more sprites expands the
 * pool automatically. Each PNG is 112×96: 3 direction rows (down, up, right) × 7
 * frames (16×32 each).
 */
export async function loadCharacterSprites(
  ...
  try {
    ...
    if (!fs.existsSync(charDir)) {
      console.log(`[AssetLoader] No characters dir at: ${charDir}`);
      return null;
    }
    ...
    if (charFiles.length === 0) {
      console.log(`[AssetLoader] No char_N.png found in: ${charDir}`);
      return null;
    }
    ...
    for (const { filename } of charFiles) {
      characters.push(decodeCharacterPng(fs.readFileSync(path.join(charDir, filename))));
    }

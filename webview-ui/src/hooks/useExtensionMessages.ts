...
import { getAssignment } from '../characterAssignments.js';
...
function saveAgentSeats(os: OfficeState): void {
  ...
  for (const ch of os.characters.values()) {
    ...
    // Persist the permanent work desk, not a transient break-room seat.
    seats[ch.id] = {
      palette: ch.palette,
      hueShift: ch.hueShift,
      seatId: ch.workSeatId ?? ch.seatId,
    };
  }
  ...
  useEffect(() => {
    ...
    const handler = (msg: any) => {
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        // If the layout (and seats) are already built, add agents directly.
        // Otherwise buffer them — layoutLoaded flushes pendingAgents once seats exist.
        // (existingAgents can arrive AFTER layoutLoaded when connecting to a server
        //  that already has agents, e.g. a reload or a second window.)
        ...
        os.setAgentTokens(
          ...
          msg.cacheReadTokens as number | undefined,
          msg.cacheWriteTokens as number | undefined,
        );
        const model = msg.model as string | undefined;
        os.setAgentModel(id, model);
        // Apply the user's project × model character assignment (if any) now
        // that the model is known.
        const folder = os.getAgentFolderName(id);
        const palette = getAssignment(folder, model);
        // Ignore a stale pin whose sprite no longer exists (e.g. char_N.png
        // removed) rather than silently wrapping to a different character.
        if (palette !== undefined && palette < getLoadedCharacterCount()) {
          os.reskinAgent(id, palette, 0);
        }
      }
    };

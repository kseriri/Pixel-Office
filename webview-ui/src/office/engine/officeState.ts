...
export class OfficeState {
  ...
  pets: Pet[] = [];
  /** Named rooms from the layout (work booths + break room). Empty for legacy layouts. */
  rooms: Room[] = [];
  /** seat uid → room id (which room each seat sits in) */
  seatRoom: Map<string, string> = new Map();
  /** room id → walkable tiles inside that room (for in-room wandering) */
  private roomWalkable: Map<string, Array<{ col: number; row: number }>> = new Map();
  /** project key (folderName) → assigned work room id */
  private projectRoom: Map<string, string> = new Map();
  ...
  /** Rebuild room lookups (rooms, seat→room, per-room walkable tiles) from the
   *  current layout + seats. Preserves project→room assignments. */
  private rebuildRooms(): void {
    this.rooms = this.layout.rooms ?? [];
    this.seatRoom = new Map();
    this.roomWalkable = new Map();
    if (this.rooms.length === 0) return;
    for (const [uid, seat] of this.seats) {
      const room = this.roomOf(seat.seatCol, seat.seatRow);
      if (room) this.seatRoom.set(uid, room.id);
    }
    for (const room of this.rooms) {
      ...
      for (const t of this.walkableTiles) {
        if (
          t.col >= room.col &&
          t.col < room.col + room.w &&
          t.row >= room.row &&
          t.row < room.row + room.h
        ) {
          tiles.push(t);
        }
      }
      this.roomWalkable.set(room.id, tiles);
    }
  }

  /** The break room, or null if the layout has none */
  private get breakRoom(): Room | null {
    return this.rooms.find((r) => r.kind === 'break') ?? null;
  }

  /** Which room contains a tile, or null */
  private roomOf(col: number, row: number): Room | null {
    for (const room of this.rooms) {
      if (col >= room.col && col < room.col + room.w && row >= room.row && row < room.row + room.h) {
        return room;
      }
    }
    return null;
  ...
  /** Get (or allocate) the work room assigned to a project. Rooms are handed out
   *  in layout order; once every work room is taken, projects share the least-used one. */
  private assignProjectRoom(projectKey: string): string | null {
    const existing = this.projectRoom.get(projectKey);
    if (existing) return existing;
    const workRooms = this.rooms.filter((r) => r.kind === 'work');
    if (workRooms.length === 0) return null;
    const used = new Set(this.projectRoom.values());
    const free = workRooms.find((r) => !used.has(r.id));
    let chosen = free;
    if (!chosen) {
      // Every work room is taken — share the least-populated one (by current
      // non-sub-agent occupants) rather than always overloading the first.
      const count = new Map<string, number>();
      for (const ch of this.characters.values()) {
        if (ch.isSubagent || !ch.homeRoomId) continue;
        count.set(ch.homeRoomId, (count.get(ch.homeRoomId) ?? 0) + 1);
      }
      chosen = workRooms.reduce((a, b) =>
        (count.get(a.id) ?? 0) <= (count.get(b.id) ?? 0) ? a : b,
      );
    }
    this.projectRoom.set(projectKey, chosen.id);
    return chosen.id;
  }

  /** Find a free seat within a specific room, preferring PC-facing seats. */
  private findFreeSeatInRoom(roomId: string): string | null {
    const pcSeats = new Set(this.pcFacingSeatUids());
    let fallback: string | null = null;
    for (const [uid, seat] of this.seats) {
      if (seat.assigned) continue;
      if (this.seatRoom.get(uid) !== roomId) continue;
      if (pcSeats.has(uid)) return uid;
      if (fallback === null) fallback = uid;
    }
    return fallback;
  }

  /** Find a free seat in the break room, or null. */
  private findFreeBreakSeat(): string | null {
    const br = this.breakRoom;
    if (!br) return null;
    for (const [uid, seat] of this.seats) {
      if (seat.assigned) continue;
      if (this.seatRoom.get(uid) === br.id) return uid;
    }
    return null;
  }
  ...
  rebuildFromLayout(layout: OfficeLayout, shift?: { col: number; row: number }): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      if (!kept) ch.seatId = null; // will be reassigned below
      // Also keep the permanent work desk reserved. While an agent rests in the
      // break room its seatId is a break seat, so the work desk (workSeatId) must
      // be re-reserved here or another agent could be assigned to it.
      if (ch.workSeatId && ch.workSeatId !== ch.seatId) {
        const ws = this.seats.get(ch.workSeatId);
        if (ws && !ws.assigned) ws.assigned = true;
        else if (!ws) ch.workSeatId = null; // desk no longer exists
      }
    }
  ...
  /** Set of seat uids that face electronics (PCs) — used to prefer "desk" seats. */
  private pcFacingSeatUids(): Set<string> {
  ...
  private findFreeSeat(): string | null {
    ...
    const pcFacing = this.pcFacingSeatUids();
    for (const [uid, seat] of this.seats) {
      if (seat.assigned) continue;
      (pcFacing.has(uid) ? pcSeats : otherSeats).push(uid);
    }
  ...
  addAgent(
    ...
    //  1) persisted preferred seat (keeps agents on their desk across reloads)
    //  2) a free seat in the agent's project room (same project → same room)
    //  3) any free seat (fallback / no rooms defined)
    ...
    if (preferredSeatId && this.seats.has(preferredSeatId)) {
      ...
      if (!seat.assigned) {
        ...
        homeRoomId = this.seatRoom.get(preferredSeatId) ?? null;
        // Re-bind this project to the room its persisted seat lives in.
        if (folderName && homeRoomId) this.projectRoom.set(folderName, homeRoomId);
      }
    }
    if (!seatId && this.rooms.length > 0 && folderName) {
      const roomId = this.assignProjectRoom(folderName);
      if (roomId) {
        const roomSeat = this.findFreeSeatInRoom(roomId);
        if (roomSeat) {
          seatId = roomSeat;
          homeRoomId = roomId;
        }
    ...
    if (!seatId) {
      ...
      if (seatId) homeRoomId = this.seatRoom.get(seatId) ?? null;
    }
  ...
  removeAgent(id: number): void {
    ...
    // Free seat(s) and clear selection immediately. While resting, seatId is a
    // break seat and workSeatId is still reserved — free both.
    if (ch.seatId) {
      const seat = this.seats.get(ch.seatId);
  ...
  reassignSeat(agentId: number, seatId: string): void {
    ...
    // Unassign old seat(s): the current seat AND the reserved work desk (which is
    // a different seat while the agent is resting in the break room).
    if (ch.seatId) {
      const old = this.seats.get(ch.seatId);
      if (old) old.assigned = false;
    }
    if (ch.workSeatId && ch.workSeatId !== ch.seatId) {
      const oldWork = this.seats.get(ch.workSeatId);
      if (oldWork) oldWork.assigned = false;
    }
    seat.assigned = true;
    ch.seatId = seatId;
    // A manual reassignment sets a new permanent work desk (and re-binds the room).
    ch.workSeatId = seatId;
    ch.homeRoomId = this.seatRoom.get(seatId) ?? null;
    ch.onBreak = false;
  ...
  /** Walk a character toward its current seatId (or sit immediately if already there). */
  private pathAgentToSeat(ch: Character): void {
  ...
  /** Tiles a character may wander over while idle: constrained to its current
   *  room (break room while resting, else its project room). */
  private wanderTilesFor(ch: Character): Array<{ col: number; row: number }> {
    if (this.rooms.length === 0) return this.walkableTiles;
    const roomId = ch.onBreak ? (this.breakRoom?.id ?? null) : ch.homeRoomId;
    if (roomId) {
      const tiles = this.roomWalkable.get(roomId);
  ...
  setAgentActive(id: number, active: boolean): void {
    ...
    const useRooms = !ch.isSubagent && this.rooms.length > 0 && this.breakRoom !== null;

    if (active) {
      if (ch.onBreak) {
        // Resumed work → free the break seat and head back to the work desk.
        // Runs regardless of useRooms so a vanished break room can't wedge the agent.
        if (ch.seatId && ch.seatId !== ch.workSeatId) {
          const bs = this.seats.get(ch.seatId);
          if (bs) bs.assigned = false;
        }
        ch.onBreak = false;
        ch.seatId = ch.workSeatId;
        if (ch.workSeatId) {
          const ws = this.seats.get(ch.workSeatId);
          if (ws) ws.assigned = true;
        }
        ch.seatTimer = 0;
        this.pathAgentToSeat(ch);
      }
      // else: the character FSM's "became active → path to seat" handles the walk.
    } else if (useRooms && ch.workSeatId && !ch.onBreak) {
      // Finished a turn → walk to the break room to rest.
      const breakSeat = this.findFreeBreakSeat();
      if (breakSeat) {
        this.seats.get(breakSeat)!.assigned = true;
        ch.onBreak = true;
        ch.seatId = breakSeat;
        ch.seatTimer = 0;
        this.pathAgentToSeat(ch);
      } else {
        // Break room full — fall back to resting in place.
        ch.seatTimer = -1;
        ch.path = [];
        ch.moveProgress = 0;
      }
  ...
  setAgentTokens(
    ...
    if (cacheReadTokens !== undefined) ch.cacheReadTokens = cacheReadTokens;
    if (cacheWriteTokens !== undefined) ch.cacheWriteTokens = cacheWriteTokens;
  }

  /** Record the AI model an agent is using (also mirrored onto its sub-agents). */
  setAgentModel(id: number, model: string | undefined): void {
    if (!model) return;
    const ch = this.characters.get(id);
    if (!ch) return;
    ch.model = model;
    // Propagate to this agent's sub-agents so they share the parent's model badge.
    for (const [subId, meta] of this.subagentMeta) {
      if (meta.parentAgentId === id) {
        const sub = this.characters.get(subId);
        if (sub) sub.model = model;
      }
    }
  }
  ...
  update(dt: number): void {
    ...
    for (const ch of this.characters.values()) {
      ...
      // Idle wander is constrained to the character's current room.
      this.withOwnSeatUnblocked(ch, () =>
        updateCharacter(ch, dt, this.wanderTilesFor(ch), this.seats, this.tileMap, this.blockedTiles),
      );
  ...
  /** Folder (project) name a character belongs to, or undefined. */
  getAgentFolderName(id: number): string | undefined {
    return this.characters.get(id)?.folderName;
  }

  /** Re-skin a live character (palette + hue). Used when a project×model
   *  character assignment resolves once the agent's model becomes known.
   *  The renderer keys sprites by palette:hueShift, so this takes effect on the
   *  next frame with no cache work. Sub-agents inherit their parent, so skip. */
  reskinAgent(id: number, palette: number, hueShift = 0): void {
    const ch = this.characters.get(id);

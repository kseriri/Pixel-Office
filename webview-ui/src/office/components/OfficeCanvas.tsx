...
export function OfficeCanvas({
  ...
  // Pan state (imperative, no re-renders). Panning = middle-mouse drag, or
  // Space + left-drag (Figma-style).
  const isPanningRef = useRef(false);
  const spaceHeldRef = useRef(false);
  const didPanRef = useRef(false); // set while panning so the trailing click is ignored
  ...
  // Space bar toggles Figma-style pan mode (hold Space, then left-drag to pan).
  useEffect(() => {
    ...
    // End a pan even if the mouse is released outside the canvas.
    const onWindowMouseUp = () => {
      if (!isPanningRef.current) return;
      isPanningRef.current = false;
      const canvas = canvasRef.current;
      if (canvas) {
        canvas.style.cursor = spaceHeldRef.current ? 'grab' : isEditMode ? 'crosshair' : 'default';
      }
    };
    // Losing focus (Alt-Tab) can swallow the Space keyup — reset pan-ready state.
    const onBlur = () => {
      spaceHeldRef.current = false;
      isPanningRef.current = false;
      const canvas = canvasRef.current;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      // Handle middle-mouse / Space+left panning
      if (isPanningRef.current) {
        const dpr = window.devicePixelRatio || 1;
        const dx = (e.clientX - panStartRef.current.mouseX) * dpr;
        const dy = (e.clientY - panStartRef.current.mouseY) * dpr;
        if (dx !== 0 || dy !== 0) didPanRef.current = true; // suppress the trailing click
        panRef.current = clampPan(panStartRef.current.panX + dx, panStartRef.current.panY + dy);
        return;
      }
      // Space held (pan-ready): keep the grab cursor, skip hover/tile logic
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      unlockAudio();
      // Panning: middle mouse (button 1), or Space + left mouse (button 0).
  ...
  const handleMouseUp = useCallback(
    (e: React.MouseEvent) => {
      // End any pan (middle-mouse, or Space + left-drag)
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      // Ignore the click the browser fires at the end of a Space+left-drag pan,
      // so panning doesn't accidentally (de)select an agent or reassign a seat.

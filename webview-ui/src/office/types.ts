...
export const RoomKind = { WORK: 'work', BREAK: 'break' } as const;
export type RoomKind = (typeof RoomKind)[keyof typeof RoomKind];

/** A rectangular region of the office grid. Work rooms hold one project's
 *  agents; the break room is where idle agents rest. Coordinates are the
 *  walkable interior (excludes surrounding walls). */
...
export interface OfficeLayout {
  ...
  pets?: PlacedPet[];
  /** Named rooms (work booths + break room). Optional: legacy layouts have none,
   *  in which case the whole office behaves as one open room. */
  rooms?: Room[];
}
...
export interface Character {
  ...
  isActive: boolean;
  /** Current target seat uid (work desk while active, break-room seat while resting), or null */
  seatId: string | null;
  /** Permanent work desk seat in the agent's project room. Persisted across sessions.
   *  Stays reserved even while the agent is resting in the break room. */
  workSeatId: string | null;
  /** Id of the work room this agent belongs to (its project's room), or null */
  homeRoomId: string | null;
  /** True while the agent has finished its turn and is resting in the break room */
  onBreak: boolean;
  ...
  outputTokens: number;
  /** Cumulative seconds spent actively working (for the utilization bar) */
  activeSec: number;
  /** Cumulative seconds spent waiting on the user / a permission prompt */
  waitingSec: number;
  /** Cumulative seconds spent idle / resting */
  idleSec: number;
  /** Cumulative cache-read input tokens (billed ~0.1x input rate) */
  cacheReadTokens?: number;
  /** Cumulative cache-write input tokens (billed ~1.25x input rate) */
  cacheWriteTokens?: number;
  /** AI model id in use (e.g. "claude-fable-5"), or undefined until known */
  model?: string;
}

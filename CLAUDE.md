...
- **VS Code extension** (`.vsix`) — publisher `kseriri`, bundles VS Code adapter + webview SPA + assets + hook scripts.
- **npm package** (`pixel-office`) — `node dist/cli.js [--port 3100]` (bin `pixel-office`) runs the Fastify server and serves the SPA on the same port. Bundles CLI + webview SPA + assets + hook scripts + `core/asyncapi.yaml` (so third-party clients can regenerate from it).
...
- Package: `pixel-office` (publisher `kseriri`, CLI bin: `pixel-office`)
- GitHub: `https://github.com/kseriri/Pixel-Office`
- Upstream: forked from `pixel-agents-hq/pixel-agents` (Pablo De Lucca, MIT)
- License: MIT
